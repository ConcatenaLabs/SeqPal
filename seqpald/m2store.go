...
type ScreeningResult struct {
	...
	LastScreenedAt int64  `json:"last_screened_at"`
	Matched        bool   `json:"matched"`
	MatchedEntry   string `json:"matched_entry,omitempty"`
}

func (s *Store) UpsertScreening(aid string, r ScreeningResult) error {
	_, err := s.db.Exec(
		`INSERT INTO screening (aid, list, last_screened_at, matched, matched_entry)
         VALUES (?,?,?,?,?)
         ON CONFLICT(aid, list) DO UPDATE SET
            last_screened_at=excluded.last_screened_at, matched=excluded.matched,
            matched_entry=excluded.matched_entry`,
		aid, r.List, r.LastScreenedAt, boolInt(r.Matched), r.MatchedEntry)
	return err
}

func (s *Store) ScreeningByAID(aid string) ([]ScreeningResult, error) {
	rows, err := s.db.Query(
		`SELECT list, last_screened_at, matched, matched_entry FROM screening WHERE aid = ? ORDER BY list`, aid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var matched int
		if err := rows.Scan(&r.List, &r.LastScreenedAt, &matched, &r.MatchedEntry); err != nil {
			return nil, err
		}
		r.Matched = matched != 0
		out = append(out, r)
...
type ReviewItem struct {
	...
	State        string `json:"state"`       // pending | cleared | confirmed
	Disposition  string `json:"disposition"` // auto-reviewer TEST-persona hint: "" | clear | confirm
...
func (s *Store) InsertReview(item *ReviewItem) error {
	_, err := s.db.Exec(
		`INSERT INTO review_queue (id, aid, list, matched_entry, state, disposition, created_at)
         VALUES (?,?,?,?,?,?,?)`,
		item.ID, item.AID, item.List, item.MatchedEntry, item.State, item.Disposition, item.CreatedAt)
	return err
}

func (s *Store) ReviewsByState(state string) ([]*ReviewItem, error) {
	rows, err := s.db.Query(
		`SELECT id, aid, list, matched_entry, state, disposition, created_at, decided_at, decided_by, note
         FROM review_queue WHERE state = ? ORDER BY created_at`, state)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) PendingReviewsByAID(aid string) ([]*ReviewItem, error) {
	rows, err := s.db.Query(
		`SELECT id, aid, list, matched_entry, state, disposition, created_at, decided_at, decided_by, note
         FROM review_queue WHERE aid = ? AND state = 'pending' ORDER BY created_at`, aid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) ReviewByID(id string) (*ReviewItem, error) {
	rows, err := s.db.Query(
		`SELECT id, aid, list, matched_entry, state, disposition, created_at, decided_at, decided_by, note
...
func scanReviews(rows *sql.Rows) ([]*ReviewItem, error) {
	out := []*ReviewItem{}
	for rows.Next() {
		var it ReviewItem
		if err := rows.Scan(&it.ID, &it.AID, &it.List, &it.MatchedEntry, &it.State, &it.Disposition,
			&it.CreatedAt, &it.DecidedAt, &it.DecidedBy, &it.Note); err != nil {
			return nil, err
		}
		out = append(out, &it)
	}
...
// DecideReview marks a review-queue item resolved. It only transitions items
// still pending, so two reviewers (a human and the auto-reviewer) cannot both
// decide the same item.
func (s *Store) DecideReview(id, state, by, note string) (bool, error) {
	res, err := s.db.Exec(
		`UPDATE review_queue SET state = ?, decided_at = ?, decided_by = ?, note = ?
         WHERE id = ? AND state = 'pending'`,
		state, time.Now().Unix(), by, note, id)
	if err != nil {
		return false, err
	}
